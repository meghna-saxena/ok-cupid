...
export default class Home extends React.Component<{}, IState> {
  ...
  private handleFilterChange = (value: boolean | any, term: string) => {
    console.log("value", value);
    const filteredOption =
      typeof value !== "boolean" ? { min: value[0], max: value[1] } : value;

    this.setState(
      {
        filters: { ...this.state.filters, [term]: filteredOption }
      },
      () => {
  ...
  private async searchFilteredUsers() {
    const { filters } = this.state;

    const response = await axios.post(
      "http://localhost:8080/api/users/filter",
      {
        ...filters
      }
    );
  ...
  public render() {
    ...
    const userProfiles =
      filteredUsers && filteredUsers.length >= 1 ? filteredUsers : users;
